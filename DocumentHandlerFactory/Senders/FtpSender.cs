...
namespace DocumentHandlerFactory.Senders
{
    public class FtpSender : ISender
    {
        ...
        {
            await ExecuteAsync(FileExistsMethod, path);
            return true;
        }
        ...
        public async Task UploadFileAsync(string path, byte[] buffer, CancellationToken cancellationToken = default)
            => await ExecuteAsync(WebRequestMethods.Ftp.UploadFile, path, buffer);
        ...
        public async Task DeleteFileAsync(string path, CancellationToken cancellationToken = default)
            => await ExecuteAsync(WebRequestMethods.Ftp.DeleteFile, path);
        ...
        public async Task CopyFileAsync(string sourceFilePath, string destinationFolder, CancellationToken cancellationToken = default)
        {
            byte[] bytes = await ReadAllBytesAsync(sourceFilePath, cancellationToken);
            await UploadFileAsync(destinationFolder, bytes, cancellationToken);
        }
        ...
        protected void Execute(string method, string path, byte[] buffer = null)
        {
        ...
        protected async Task ExecuteAsync(string method, string path, byte[] buffer = null)
        {
            FtpWebRequest req = GetRequest(path, method);
